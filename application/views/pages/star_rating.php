<main class="flex-1 p-8">
	<div x-data="{ rating: 0, hoverRating: 0, ratings: [ 1, 2, 3, 4, 5]}" class="flex items-center space-x-1">
		<template x-for="star in ratings">
			<button @click="rating = star" @mouseover="hoverRating = star" @mouseleave="hoverRating = rating" aria-hidden="true" class="p-1 rounded-sm focus:outline-none focus:shadow-outline">
				<svg :class="{ 'text-gray-600' : hoverRating >= star, 'text-yellow-400' : rating >= star }" class="w-8 text-gray-400 transition duration-150 cursor-pointer fill-current" viewBox="0 -10 511.991 511" xmlns="http://www.w3.org/2000/svg">
